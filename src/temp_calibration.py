...
def fit_center(data, center_estimate=False, power=False,
               dwell=False, num=False, plot=False, savefig=False, verbose=False):
    '''
    ...
    if center_estimate:
        ...
        pfit, _ = fit_with(gaussian_shift, data, param=params, verbose=verbose)
    else:
        pfit, _ = fit_with(gaussian_shift, data, param_estimator=moments, verbose=verbose)
...
def estimate_noise(img):
    '''
    Estimating the noise by calculating the difference between blurred img and the
    original one.
    '''
...
def surface_plot (matrix, fit, **kwargs):
    ''' quick function for plotting 3D surfaces of a given matrix and the fit'''
...
def fit_with(func, data, mask=None, param_estimator=None,
    ...
    '''
    Fit given data with given function and return the fitting paramters and square root
    of sum of error.
    ...
    Keyward Arguments:
        mask: a boolean array with the same shape of the data to cover bad spots
        param_estimator: a function that estimates the param with given data
        param: array of real number as the starting point of the least square solver
